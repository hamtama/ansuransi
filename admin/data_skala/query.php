...
$query_data = ($search !== '') ? "SELECT (ROW_NUMBER() OVER (Order by id_skala)) as nomor, `*` FROM $tb_name WHERE skala LIKE '%$search%' ORDER BY $order_field $order_ascdesc LIMIT $start, $limit" : "SELECT (ROW_NUMBER() OVER (Order by id_skala)) as nomor, `*` FROM $tb_name ORDER BY $order_field $order_ascdesc LIMIT $start, $limit";
$query_filter = ($search !== '') ? "SELECT COUNT(*) FROM $tb_name WHERE status LIKE '%$search%'" : "SELECT COUNT(*) FROM $tb_name";
