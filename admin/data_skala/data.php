    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            ...
            <div class="card-box mb-30">
                <div class="pd-20">
                    <h4 class="text-blue h4">Data Skala</h4>
                </div>
                <div class="pb-20">
                    <table id="datatabel" class="data-table table stripe hover" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle">Skala</th>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="form_skala" class="needs-validation" novalidate>
                                    <div class="form-group">
                                        <label class="col-sm-12 col-form-label required pl-0">Data Skala</label>
                                        <input type="text" id="skala" name="skala" class="form-control" required />
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-12 col-form-label required pl-0">Data Status</label>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            "columns": [{
                ...
                },
                {
                    "render": function(data, type, row) {
                        var html = '<input type="hidden" name="id_skala" value="' + row.id_skala + '"/>'
        ...
        $('#datatabel tbody').on('click', '.editBtn', function() {
            $('#mode').val("update");
            $('#myLargeModalLabel').text('Edit Data Skala');
            var data = ($('#datatabel').hasClass('collapsed')) ? tabel.row($(this).closest('tr').index() - 1).data() : tabel.row($(this).closest('tr').index()).data();
            var idskala = $(this).closest('tr').find('td input[name="id_skala"]').val();
            $('#id').val(idskala);
            $('#skala,#status').addClass('is-valid');
        ...
        $('#datatabel tbody').on('click', '.deleteBtn', function() {
            var data = ($('#datatabel').hasClass('collapsed')) ? tabel.row($(this).closest('tr').index() - 1).data() : tabel.row($(this).closest('tr').index()).data();
            var idskala = $(this).closest('tr').find('td input[name="id_skala"]').val();
            if (confirm("Hapus data Skala : '" + data.status + "'?") === true) {
                $.post("crudData.php", {
                    id: idskala,
                    mode: "delete"
                }, function(data) {
        ...
        $('.tambah').click(function() {
            $('#mode').val("create");
            $('#id').val("");
            $('#myLargeModalLabel').text('Tambah Data Keterangan Kecelakaan');
            myModal.toggle();
            $("#batal").on('click', function() {
                myModal.toggle();
            });
        ...
        $('#simpan').click(function() {
            if ($('#skala,#status').hasClass('is-valid')) {
                $('#form_skala').addClass('was-validated');
                $('#form_skala').submit();
            }
        });

        $('#form_skala').on('submit', function(e) {
        ...
        $('#Medium-modal').on('hidden.bs.modal', function() {
            $('#skala,#status').removeClass('is-valid');
            $('#skala,#status').val('');
            tabel.draw();
            // clear();
        });
