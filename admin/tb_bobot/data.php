                            ...
                            <div class="card-box">
                                <table id="datatable-responsive"
                                    ...
                                    <tbody>
                                        <?php
										$no = 1;
										$query = "SELECT * FROM tb_bobot";
										$sql_bobot = mysqli_query($mysqli, $query) or die(mysqli_error($query));
                                        ...
                                        <tr>
                                            <td style="width:10%">
                                                <a class="btn btn-warning btn-xs" data-toggle="modal" href="#myModal"
                                                    ...
                                                    data-id="<?=$row['id_bobot'] ?>"><i class="fa fa-pencil"></i></a>
                                                <button class="btn btn-danger btn-xs"
                                                    onclick="javascript:delete_id(<?=$row['id_bobot']?>)"><i
                                                        class="fa fa-trash"></i></button>
                                            </td>
                                            <td><?=$no++?>.</td>
                                            <td><?=$row['bobot']?></td>
                                            <td><?=$row['status_bobot']?></td>
                                        </tr>
                                        <?php
										}
										} 
    ...
    $('.tambah').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.modal-title').text('Tambah Data Bobot');
            }
        });
    });
});
...
$(document).ready(function() {
    $('#myModal').on('show.bs.modal', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('.fetched-data').html(data); //menampilkan data ke dalam modal
                $('.modal-title').text('Edit Data Bobot');
            }
        });
    });
});
