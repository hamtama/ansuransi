...
$cek            = mysqli_num_rows($mysqli->query("SELECT nama FROM users WHERE nama ='$_POST[nama]'"));
$cek2           = mysqli_num_rows($mysqli->query("SELECT username FROM users WHERE username = '$_POST[username]'"));
$cek3           = mysqli_num_rows($mysqli->query("SELECT email FROM users WHERE email = '$_POST[email]'"));
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    if ($cek >= 1){
        ...
        $sql = $mysqli->query("INSERT INTO users SET
            level       ='$level',
            nama        ='$nama',
            username    ='$username',
            email       ='$email',
        ...
        if ($sql) {
            ...
            echo "Input Silahkan Ulangi !",($mysqli->error);
        }
    }
}
?>