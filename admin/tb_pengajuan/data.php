                            ...
                            <div class="card-box">
                                <table id="datatable-responsive"
                                    ...
                                    <tbody>
                                        <?php
										$no = 1;
										$query = "SELECT (ROW_NUMBER() OVER (Order by id_pengajuan)) as nomor, id_pengajuan, no_pengajuan,  
                                        ...
                                        no_telp_pengaju from tb_pengajuan a  JOIN wilayah_2020 b ON b.kode = a.id_prov_pengaju 
                                         ...
                                         LEFT JOIN tb_pekerjaan f ON f.id_pekerjaan = a.pekerjaan_pengaju";
                                        ...
                                        <tr>
                                            <td style="width:10%">
                                                ...
                                                <button class="btn btn-danger btn-xs"
                                                    onclick="javascript:delete_id(<?=$row[1]?>)"><i
                                            ...
                                            <td><?= $row['tempat_lhr_pengaju'].",".$row['tgl_lahir'] ?></td>
                                            <td><?= $row['ktp_pengaju'] ?></td>
                                            <td><?= $row['alamat_pengaju'].", ".$row['kel'].", ".$row['kec'].", ".$row['kab'].", ".$row['prov'] ?>
                                            </td>
                                            <td><?= $row['umur_pengaju'] ?></td>
                                            <td><?= $row['jk_pengaju'] ?></td>
                                            <td><?= $row['pekerjaan'] ?></td>
                                            <td><?= $row['no_telp_pengaju'] ?></td>
...
function delete_id(id) {
    if (confirm('Anda Serius Untuk Menghapus Data ? Menghapus Data Akan Menhapus Data Terkait Lainnya')) {
