...
$query_data = ($search !== '') ? "SELECT (ROW_NUMBER() OVER (Order by id_gagal)) as nomor, `*` FROM $tb_name WHERE data_gagal LIKE '%$search%' ORDER BY $order_field $order_ascdesc LIMIT $start, $limit" : "SELECT (ROW_NUMBER() OVER (Order by id_gagal)) as nomor, `*` FROM $tb_name ORDER BY $order_field $order_ascdesc LIMIT $start, $limit";
$query_filter = ($search !== '') ? "SELECT COUNT(*) FROM $tb_name WHERE data_gagal LIKE '%$search%'" : "SELECT COUNT(*) FROM $tb_name";
