    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            ...
            <div class="card-box mb-30">
                <div class="pd-20">
                    <h4 class="text-blue h4">Data Keterangan Gagal</h4>
                </div>
                <div class="pb-20">
                    <table id="datatabel" class="data-table table stripe hover" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle">Keterangan Gagal</th>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="form_gagal" class="needs-validation" novalidate>
                                    <div class="form-group">
                                        <label class="col-sm-12 col-form-label required pl-0">Data Keterangan Gagal</label>
                                        <input type="text" id="ket_gagal" name="ket_gagal" class="form-control" required />
                                    </div>
                                    <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            "columns": [{
                ...
                },
                {
                    "render": function(data, type, row) {
                        var html = '<input type="hidden" name="id_gagal" value="' + row.id_gagal + '"/>'
        ...
        $('#datatabel tbody').on('click', '.editBtn', function() {
            ...
            var idGagal = $(this).closest('tr').find('td input[name="id_gagal"]').val();
            $('#id').val(idGagal);
            $('#ket_gagal').addClass('is-valid');
            $('#ket_gagal').val(data.data_gagal);
        ...
        $('#datatabel tbody').on('click', '.deleteBtn', function() {
            var data = ($('#datatabel').hasClass('collapsed')) ? tabel.row($(this).closest('tr').index() - 1).data() : tabel.row($(this).closest('tr').index()).data();
            var idGagal = $(this).closest('tr').find('td input[name="id_gagal"]').val();
            if (confirm("Hapus data Keterangan Gagal : \n'" + data.data_gagal + "'?") === true) {
                $.post("crudData.php", {
                    id: idGagal,
                    mode: "delete"
                }, function(data) {
        ...
        $('#simpan').click(function() {
            if ($('#ket_gagal').hasClass('is-valid')) {
                $('#form_gagal').addClass('was-validated');
                $('#form_gagal').submit();
            }
        });

        $('#form_gagal').on('submit', function(e) {
        ...
        $('#Medium-modal').on('hidden.bs.modal', function() {
            $('#ket_gagal').removeClass('is-valid');
            $('#ket_gagal').val('');
            tabel.draw();
            // clear();
        });
