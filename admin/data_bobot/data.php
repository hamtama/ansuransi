    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            ...
            <div class="card-box mb-30">
                <div class="pd-20">
                    <h4 class="text-blue h4">Data Bobot</h4>
                </div>
                <div class="pb-20">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="form_bobot" class="needs-validation" novalidate>
                                    <div class="form-group">
                                        <label class="col-sm-12 col-form-label required pl-0">Data Bobot</label>
                                        <input type="text" id="bobot" name="bobot" class="form-control" required />
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-12 col-form-label required pl-0">Status Bobot</label>
                                        <input type="text" id="status_bobot" name="status_bobot" class="form-control" required />
                                    </div>
                                    <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            "columns": [{
                ...
                },
                {
                    "render": function(data, type, row) {
                        var html = '<input type="hidden" name="id_bobot" value="' + row.id_bobot + '"/>'
        ...
        $('#datatabel tbody').on('click', '.editBtn', function() {
            $('#mode').val("update");
            $('#myLargeModalLabel').text('Edit Data Bobot');
            var data = ($('#datatabel').hasClass('collapsed')) ? tabel.row($(this).closest('tr').index() - 1).data() : tabel.row($(this).closest('tr').index()).data();
            var idbobot = $(this).closest('tr').find('td input[name="id_bobot"]').val();
            $('#id').val(idbobot);
            $('#bobot,#status_bobot').addClass('is-valid');
            $('#bobot').val(data.bobot);
            $('#status_bobot').val(data.status_bobot);
            myModal.toggle();
        ...
        $('#datatabel tbody').on('click', '.deleteBtn', function() {
            var data = ($('#datatabel').hasClass('collapsed')) ? tabel.row($(this).closest('tr').index() - 1).data() : tabel.row($(this).closest('tr').index()).data();
            var idbobot = $(this).closest('tr').find('td input[name="id_bobot"]').val();
            if (confirm("Hapus data Bobot : '" + data.status_bobot + "'?") === true) {
                $.post("crudData.php", {
                    id: idbobot,
                    mode: "delete"
                }, function(data) {
        ...
        $('.tambah').click(function() {
            $('#mode').val("create");
            $('#id').val("");
            $('#myLargeModalLabel').text('Tambah Data Bobot');
            myModal.toggle();
            $("#batal").on('click', function() {
                myModal.toggle();
            });
        ...
        $('#simpan').click(function() {
            if ($('#bobot,#status_bobot').hasClass('is-valid')) {
                $('#form_bobot').addClass('was-validated');
                $('#form_bobot').submit();
            }
        });
        ...
        $('#Medium-modal').on('hidden.bs.modal', function() {
            $('#bobot,#status_bobot').removeClass('is-valid');
            $('#bobot,#status_bobot').val('');
            tabel.draw();
            // clear();
        });
