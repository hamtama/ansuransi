...
$query_data = ($search !== '') ? "SELECT (ROW_NUMBER() OVER (Order by id_bobot)) as nomor, `*` FROM $tb_name WHERE bobot LIKE '%$search%' ORDER BY $order_field $order_ascdesc LIMIT $start, $limit" : "SELECT (ROW_NUMBER() OVER (Order by id_bobot)) as nomor, `*` FROM $tb_name ORDER BY $order_field $order_ascdesc LIMIT $start, $limit";
